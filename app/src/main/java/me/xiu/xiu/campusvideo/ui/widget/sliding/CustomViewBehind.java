...
public class CustomViewBehind extends FrameLayout {
    ...
    private int mTouchMode = SlidingLayout.TOUCHMODE_MARGIN;
    ...
    private SlidingLayout.CanvasTransformer mTransformer;
    ...
    public void setCanvasTransformer(SlidingLayout.CanvasTransformer t) {
        mTransformer = t;
    }
    ...
    /**
     * Sets the secondary (right) menu for use when setMode is called with SlidingLayout.LEFT_RIGHT.
     *
    ...
    public void setMode(int mode) {
        if (mode == SlidingLayout.LEFT || mode == SlidingLayout.RIGHT) {
            if (mContent != null)
    ...
    public int getMenuPage(int page) {
        ...
        if (mMode == SlidingLayout.LEFT && page > 1) {
            return 0;
        } else if (mMode == SlidingLayout.RIGHT && page < 1) {
            return 2;
        } else {
            return page;
        }
    ...
    public int getAbsLeftBound(View content) {
        if (mMode == SlidingLayout.LEFT || mMode == SlidingLayout.LEFT_RIGHT) {
            return content.getLeft() - getBehindWidth();
        } else if (mMode == SlidingLayout.RIGHT) {
            return content.getLeft();
        }
    ...
    public int getAbsRightBound(View content) {
        if (mMode == SlidingLayout.LEFT) {
            return content.getLeft();
        } else if (mMode == SlidingLayout.RIGHT || mMode == SlidingLayout.LEFT_RIGHT) {
            return content.getLeft() + getBehindWidth();
        }
        return 0;
    }
    ...
    public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
        switch (mTouchMode) {
            case SlidingLayout.TOUCHMODE_FULLSCREEN:
                return true;
            case SlidingLayout.TOUCHMODE_MARGIN:
    ...
    public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
        if (mMode == SlidingLayout.LEFT || (mMode == SlidingLayout.LEFT_RIGHT && currPage == 0)) {
            return x >= content.getLeft();
        } else if (mMode == SlidingLayout.RIGHT || (mMode == SlidingLayout.LEFT_RIGHT && currPage == 2)) {
